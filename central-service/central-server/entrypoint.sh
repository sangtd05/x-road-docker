...
then
    ...
    chmod 0600 /etc/xroad/autologin
...
if ! crudini --get /etc/xroad/conf.d/local.ini signer enforce-token-pin-policy &>/dev/null; then
  crudini --set /etc/xroad/conf.d/local.ini signer enforce-token-pin-policy "false"
fi
...
# Khởi tạo softtoken nếu cần
if [ ! -f /var/lib/softhsm/tokens/token.db ]; then
  log "Initializing SoftHSM token"
  mkdir -p /var/lib/softhsm/tokens
  softhsm2-util --init-token --slot 0 --label "X-Road" --pin "$XROAD_TOKEN_PIN" --so-pin "$XROAD_TOKEN_PIN"
  chown -R xroad:xroad /var/lib/softhsm
fi

# Đảm bảo quyền cho các thư mục và tệp quan trọng
chown -R xroad:xroad /etc/xroad
chown -R xroad:xroad /var/lib/xroad

# Khởi tạo transport keys
